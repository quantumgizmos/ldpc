...
TEST(TestSparseMatrix, InitialisationAndAllocationStack)
{
    ...
    sparse_matrix::SparseMatrix<int> pcm(3, 4); // Stack-allocated SparseMatrix
...
//         for (auto entry : matrix.iterate_column_ptr(col))
//         {
...
//         for (auto entry : matrix.reverse_iterate_column_ptr(col))
//         {
//             actual_values[i] = entry->value;
//             i++;
//         }
