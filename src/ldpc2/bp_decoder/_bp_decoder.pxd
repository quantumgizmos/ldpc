...
cdef extern from "bp.hpp":
    ...
    cdef cppclass bp_sparse:
        bp_sparse(int m, int n) except +
        bp_entry* insert_entry(int i, int j, unsigned char value)
        bp_entry* get_entry(int i, int j)
        vector[uint8_t]& mulvec(vector[uint8_t]& input_vector, vector[uint8_t]& output_vector)
        int m
        int n

    cdef cppclass bp_decoder_cpp "bp_decoder":
        bp_decoder_cpp(bp_sparse* pcm, vector[double]& error_channel, int max_iter, int bp_method, double ms_scaling_factor, int schedule, int omp_threads, vector[int] serial_schedule,int random_schedule) except +
