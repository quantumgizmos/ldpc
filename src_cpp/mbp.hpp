...
class mbp_entry : public ldpc::sparse_matrix_base::EntryBase<mbp_entry<T>> {
public:
...
class mbp_sparse : public ldpc::sparse_matrix::SparseMatrix<uint8_t, mbp_entry> {
public:
    using BASE = SparseMatrix<uint8_t, mbp_entry>;
    using BASE::row_heads;
    using BASE::column_heads;
    using BASE::m;
    using BASE::n;
    ...
    vector<uint8_t> &pauli_syndrome(vector<uint8_t> &input_vector, vector<uint8_t> &output_vector) {
        ...
        for (int stab = 0; stab < m; stab++) {
            ...
            row_sum = 0;
            for (const auto &e: BASE::iterate_row(stab)) {
...
public:
    ...
    mbp_decoder(mbp_sparse *matrix, vector<vector<double>> &channel_probabilities, int maximum_iterations,
                vector<vector<double>> alpha_parameter, double beta_parameter = 0.0, int bp_method = 0,
                double min_sum_gamma = 1.0) {
        ...
        for (int w = 0; w < 3; w++) {
            log_prob_ratios[w].resize(qubit_count);
        }
        candidate_syndrome.resize(stab_count);
        decoding.resize(qubit_count);
        converge = 0;
        alpha = std::move(alpha_parameter);
        beta = beta_parameter;
            ...
            for (int qubit = 0; qubit < qubit_count; qubit++) {
                ...
                if (bp_method == 0) {
                    ...
                    for (auto e: pcm->iterate_column_ptr(qubit)) {
                        ...
                        e->stab_to_qubit_msgs =
                                pow(-1, synd[stab_index]) * log((1 + stab_update_product) / (1 - stab_update_product));
                ...
                for (auto e: pcm->iterate_column_ptr(qubit)) {
                    for (int w = 0; w < 3; w++) {
                        if (e->pauli != (w + 1))
                            log_prob_ratios[w][qubit] += (1 / alpha[w][qubit]) * e->stab_to_qubit_msgs;
