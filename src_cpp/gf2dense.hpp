...
public:
    ...
    void sparse_rref(bool construct_U = true){
        ...
        std::vector<int> rr_col;

        int max_rank = std::min(this->row_count, this->col_count);

        // std::cout<<"Max rank: "<<max_rank<<std::endl;

        // print_csc(this->row_count,this->col_count,this->csc_mat);

        for(int col = 0; col<this->col_count; col++){

            rr_col = this->csc_mat[col];
            
            for(int i = 0; i<this->matrix_rank; i++){

                // std::cout<<"Col: "<<col<<" Sparse col";
                // ldpc::sparse_matrix_util::print_vector(rr_col);

                // print_csc(6,3,this->L);

                auto temp = rr_col;
                std::vector<int> remove_rows;
                std::vector<int> add_rows;
                // std::swap(rr_col[i],rr_col[this->swap_rows[i]]);
                int swap_index = vector_find(temp,this->swap_rows[i]);
                int rank_index = vector_find(temp,i);

                // std::cout<<"Swap index: "<<this->swap_rows[i]<<" Rank_index: "<<i<<std::endl;
                // std::cout<<"Swap index sparse: "<<swap_index<<" Rank_index sparse: "<<rank_index<<std::endl;

                if(rank_index != -1){
                    temp[rank_index] = this->swap_rows[i];
                }
                ...
                if(swap_index != -1){
                    ...
                    // std::cout<<"After swap: ";
                    // ldpc::sparse_matrix_util::print_vector(temp);
                    
                    for(auto it = this->L[i].begin() + 1; it != this->L[i].end(); ++it){
                        int add_row = *it;
                        // std::cout<<"Add row: " <<add_row<<std::endl;
                        if(vector_find(temp,add_row) != -1){
                            remove_rows.push_back(add_row);
                        }
                        else{
                            temp.push_back(add_row);
                        }
                    }
                ...
                rr_col.clear();
                for(auto row: temp){
                    if(vector_find(remove_rows,row) == -1){
                        rr_col.push_back(row);
                    }
                }
                // for(auto row: add_rows){
                //     rr_col.push_back(row);
                // }
            ...
            int swap_index = 0;
            for(int row: rr_col){
                if(row >= this->matrix_rank){
                    PIVOT_FOUND = true;
                    this->swap_rows.push_back(row);
                    this->pivot_cols.push_back(col);
                    
                    // std::cout<<"Pivot found: "<<row<<", "<<col<<std::endl;
            ...
            // std::cout<<"Swap index main elim stage: " << swap_index <<std::endl;
            // std::cout<<"Swap row main elim stage: " << rr_col[swap_index] <<std::endl;
            ...
            // std::swap(rr_col[this->matrix_rank],rr_col[this->swap_rows[this->matrix_rank]]);
            // int swap_index = vector_find(rr_col,this->swap_rows[this->matrix_rank]);

            int rank_index;
            if(rr_col[swap_index] == this->matrix_rank){
                rank_index = swap_index;
            }
            else{
                rank_index = vector_find(rr_col,this->matrix_rank);
            }

            int temp = rr_col[swap_index];
            rr_col[swap_index] = this->matrix_rank;
            if(rank_index!=-1){
                rr_col[rank_index] = temp;
            }

            // std::cout<<"rr_col after main elim swap: ";
            // ldpc::sparse_matrix_util::print_vector(rr_col);
            ...
            for(int row: rr_col){
                if(row > this->matrix_rank){
                    // rr_col[i] ^= 1; //we don't actually need to eliminate here.
                    this->L[this->matrix_rank].push_back(row);
                }
                else if(row < this->matrix_rank){
                    this->U[this->matrix_rank].push_back(row);
                }
                count++;
            }
...
CscMatrix sparse_kernel(int row_count, int col_count, CsrMatrix& csr_mat){
    ...
    for(int j = 0; j<col_count; j++){
        ...
        for(int i = 0; i<plu.matrix_rank; i++){
            ...
            std::vector<int> add_rows;
            // std::swap(rr_col[i],rr_col[this->swap_rows[i]]);
            int swap_index = vector_find(temp,plu.swap_rows[i]);
            int rank_index = vector_find(temp,i);
            ...
            if(rank_index != -1){
                temp[rank_index] = plu.swap_rows[i];
            }
            ...
            for(auto row: temp){
                if(vector_find(remove_rows,row) == -1){
                    if(row > plu.matrix_rank){
                        rr_col.push_back(row-plu.matrix_rank);
                    }
                }
            }

            ker.push_back(rr_col);
