...
def test_vs_ldpc_v1():
    ...
    bpd_v1 = bp_decoder_v1(
        H, error_rate=error_rate, bp_method="ms", ms_scaling_factor=1.0, max_iter=10
    )
    mc1 = MonteCarloBscSimulation(
        H, error_rate=error_rate, Decoder=bpd, target_run_count=run_count, seed=42
    )
    mc2 = MonteCarloBscSimulation(
        H, error_rate=error_rate, Decoder=bpd_v1, target_run_count=run_count, seed=42
    )
    out1 = mc1.run()
    out2 = mc2.run()
